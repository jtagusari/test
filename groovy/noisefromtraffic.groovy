...
def exec(Connection connection, input) {
  // set road table
  String tableRoads = importAndGetTable(connection, input["roadGeomPath"])

  // set building table
  String tableBuilding =  importAndGetTable(connection, input["buildingGeomPath"])

  // set reciver table
  String tableReceivers = importAndGetTable(connection, input["receiverGeomPath"])

  // set dem table
  String tableDEM = null
  if (input["demGeomPath"]) {
    tableDEM = importAndGetTable(connection, input["demGeomPath"])
  } 
  ...
  if (input["groundAbsGeomPath"]) {
    tableGroundAbs = importAndGetTable(connection, input["groundAbsGeomPath"])
  } 
  ...
  runScript(
    ...
    [
      ...
      "confMaxReflDist": input["confMaxReflDist"],
      "confThreadNumber": input["confThreadNumber"],
      "confDiffVertical": input["confDiffVertical"],
      "confDiffHorizontal": input["confDiffHorizontal"],
      "confExportSourceId": input["confExportSourceId"],
      "confHumidity": input["confHumidity"],
      "confTemperature": input["confTemperature"],
      "confFavorableOccurrencesDay": input["confFavorableOccurrencesDay"],
